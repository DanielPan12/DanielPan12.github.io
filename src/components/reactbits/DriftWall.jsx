...
const DriftWall = ({
  ...
  // Every layer between the wall and a tile (plane/col/track/tile) shares
  // transform-style: preserve-3d under one perspective, so the browser's
  // own hit-test (elementFromPoint, and therefore native click/hover
  // bubbling) doesn't reliably resolve to the tile a point is geometrically
  // inside of — it can land on an ancestor instead, even dead-center on a
  // tile. Finding the tile by testing each one's own getBoundingClientRect
  // sidesteps that entirely: it's a plain 2D containment check against the
  // browser-computed post-transform box, immune to how the 3D stack painted.
  ...
  const handleWallClick = useCallback(
    e => {
      ...
      onTileClick(item, id, tile.getBoundingClientRect());
    },
    [onTileClick, findTileAt, columnItems]
  );
