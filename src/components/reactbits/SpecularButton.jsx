...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    // Sized from the ResizeObserver's own border-box measurement rather than
    // getBoundingClientRect() — the latter reflects any ancestor CSS
    // transform (e.g. the page-transition entrance scale this button mounts
    // under), so a rect read at that instant locks the canvas to a too-small
    // size forever (ResizeObserver doesn't refire for transform-only
    // changes). The observer's box size is the untransformed layout size.
    const applySize = (w, h) => {
      sizeRef.w = w;
      sizeRef.h = h;
