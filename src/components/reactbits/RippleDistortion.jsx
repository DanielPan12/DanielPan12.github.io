...
const RippleDistortion = ({
  ...
  configRef.current = { brushSize, spread, fade, spacing, clickStrength, trigger, enabled, scrollSpeed };
  ...
  useEffect(() => {
    ...
    const loop = now => {
      ...
      if (!reduceMotion && cfg.scrollSpeed) {
        scrollOffsetRef.current = (scrollOffsetRef.current + delta * cfg.scrollSpeed) % 1;
        compositeUniforms.uScrollOffset.value = scrollOffsetRef.current;
      }
