...
const MoltenMetal = ({
  ...
  useEffect(() => {
    ...
    // contentRect ignores ancestor CSS transforms (unlike getBoundingClientRect),
    // which matters because this container mounts mid page-transition animation
    // (the page is scaled down to 0.86 during the transition) — measuring via
    // getBoundingClientRect at that moment would permanently lock the canvas to
    // 86% size, since ResizeObserver never fires again for a transform-only change.
