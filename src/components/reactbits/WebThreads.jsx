...
const WebThreads = ({
  ...
  useEffect(() => {
    ...
    // Sized from the ResizeObserver's own contentRect rather than
    // getBoundingClientRect() — the latter reflects any ancestor CSS
    // transform (e.g. PageTransition's entrance scale, still active the
    // instant this mounts on a route change), so a rect read at that exact
    // moment locks the canvas to a too-small size forever: ResizeObserver
    // doesn't refire for transform-only changes, only real layout resizes.
    // contentRect is the untransformed layout box, immune to that timing.
    ...
    // Listens on window rather than the canvas itself: this background sits
    // behind foreground page content (z-index below the actual UI), so the
    // canvas is occluded almost everywhere and would rarely receive its own
    // mousemove events. A window-level listener fires regardless of what
    // element is on top at the pointer's position, using the container's
    // own rect (same on-screen size as the canvas) to compute the UV.
    const onMouseMove = e => {
